...
class BlogSpider(scrapy.Spider):
    ...
    def parse_jutsu(self, response):
        ...
        if soup.find('div',{'id':'adLink'}):
            soup.find('div',{'id':'adLink'}).decompose()
        
        if soup.find('h2',{'id':'adLink'}):
            soup.find('h2',{'id':'adLink'}).decompose()
        
        if soup.find('a',{'id':'adLink'}):
            soup.find('a',{'id':'adLink'}).decompose()
